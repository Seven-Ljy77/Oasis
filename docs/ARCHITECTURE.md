# Mercury Linux — 项目架构方案

## Context

将 macOS 原生 RSS 阅读器 Mercury（SwiftUI + GRDB + WKWebView）移植到 Linux 平台。
选定技术栈：**Tauri 2 + Rust 后端 + React/TypeScript 前端 + WebKitGTK**。
目标：保持 macOS 版的核心产品价值（本地优先、轻量、AI 智能体），使用适合 Linux 的技术栈重写。

本文档由 Windows 版本的架构方案迁移而来——Windows 与 Linux 移植共享同一套 Rust/React 代码库
（凭据存储、路径解析等均通过跨平台 crate 实现），差异仅集中在少数系统集成点（见下文标注）。
...
| 层面 | 技术 | 说明 |
|------|------|------|
| 桌面框架 | Tauri 2.x | 系统 WebKitGTK 复用，包体较小 |
| 后端语言 | Rust (Edition 2024) | 编译为原生 Linux 二进制 |
...
| 阅读器渲染 | WebKitGTK (Tauri WebView) | 对标 WKWebView |
...
| 图表 | Recharts | 用量统计报告 |
| 凭据存储 | Secret Service (libsecret/gnome-keyring) via `keyring` crate | 对标 macOS Keychain；Windows 版对应 Credential Manager |
...
CREATE TABLE agent_provider_profile (
    ...
    api_key_ref TEXT NOT NULL,          -- OS 凭据存储引用 (Secret Service/libsecret)
    test_model TEXT,
...
### Phase 2：AI 智能体
- LLM 提供商/模型配置管理
- Secret Service (libsecret) API Key 存储
- 摘要智能体 (流式输出)
- 翻译智能体 (双语对照)
- 标签智能体 (单篇 + 批量)
- 用量追踪
...
6. 主流发行版真机/WSL2 测试（Ubuntu/Debian、Fedora）
