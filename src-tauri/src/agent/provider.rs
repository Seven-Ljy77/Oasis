...
impl OpenAIProvider {
    ...
    /// For other keys, tries the OS credential store via the keyring crate
    /// (Secret Service/libsecret on Linux, Credential Manager on Windows,
    /// Keychain on macOS), falling back to the raw `api_key_ref` value itself
    /// (for inline keys).
    async fn resolve_api_key(&self) -> Result<String, AppError> {
        ...
        // Try the OS credential store via keyring
